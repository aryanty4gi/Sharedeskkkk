...
export async function updateMyProfile(userId: string, patch: Partial<Profile>) {
  ...
  await updateProfileAction({
    data: {
      ...
      employeeCode: patch.employee_id,
...
export async function searchEverything(userId: string, q: string): Promise<SearchResults> {
  ...
  const [usersRes, msgsRes, filesRes] = await Promise.all([
    supabase
      ...
      .neq("id", userId)
      .or(
        `full_name.ilike.${term},email.ilike.${term},employee_id.ilike.${term},department.ilike.${term},designation.ilike.${term}`,
      )
      .limit(20),
    supabase
      .from("messages")
...
export async function fetchCurrentUserRole(userId: string): Promise<CompanyRole> {
  const { data, error } = await supabase
    .from("user_roles")
    .select("role")
    .eq("user_id", userId)
...
export async function adminUpdateEmployee(input: {
  ...
  await adminUpdateEmployeeAction({
    data: {
      token,
      ...input,
    },
...
export async function adminUpdateUserRole(
  ...
  await adminUpdateUserRoleAction({
    data: {
      token,
      targetUserId,
      role,
